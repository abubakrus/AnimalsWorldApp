...
class SealedTypeAdapterFactory<T : Any> private constructor(
    ...
    override fun <R : Any> create(gson: Gson, type: TypeToken<R>?): TypeAdapter<R>? {
        if (type == null || subclasses.isEmpty() || subclasses.none {
                type.rawType.isAssignableFrom(
                    it.java
                )
            }) return null
        ...
        return object : TypeAdapter<R>() {
            override fun write(writer: JsonWriter, value: R) {
                ...
                @Suppress("UNCHECKED_CAST") val delegate =
                    subclassToDelegate[srcType] as TypeAdapter<R>
            ...
            override fun read(reader: JsonReader): R {
                ...
                val labelElement =
                    element.asJsonObject.remove(typeFieldName) ?: throw JsonParseException(
                        "cannot deserialize $baseType because it does not define a field named $typeFieldName"
                    )
                val name = labelElement.asString
                val subclass = nameToSubclass[name]
                    ?: throw JsonParseException("cannot find $name subclass of $baseType")
                @Suppress("UNCHECKED_CAST")
                return (subclass.objectInstance as? R)
                    ?: (subclassToDelegate[subclass]!!.fromJsonTree(element) as R)
            }
        }
    }
